...
import { editTodoContent, updateTodoStatus } from 'container/Client/TodoList/sagas/actionCreators/todosActionCreators';
...
function TodoItem(props: {todo: Todo}) {
	...
	return (
		<div className="flex gap-2">
			<div className={`${!filter.byDate ? 'w-9/12' : 'w-full'} h-12 bg-[#e0ebff] rounded-[7px] flex justify-start items-center px-3`}>
			...
			{!filter.byDate ? <span className="w-1/4 px-2 h-12 bg-[#e0ebff] rounded-[7px] flex justify-center text-sm text-[#5b7a9d] font-semibold items-center ">{dateFormat(props.todo.created_date)}</span> : ''}
		</div>
	);
}
